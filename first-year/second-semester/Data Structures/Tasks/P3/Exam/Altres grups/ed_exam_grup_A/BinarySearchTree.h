...
public:
    ...
    BinaryTreeNode<K, V> *add(const K &k, const V &value); //inserta un element
    ...
    //retorna un vector amb tots els nodes fulla de l'arbre
    vector<BinaryTreeNode<K, V> *> getLeafs() const;

    //true si el subarbre esquerra és mirall del subarbre dret, fals altrament
    bool esArbreSimetric();

    //suma màxima dels camins possibles des de l'arrel a les fulles
    int sumaMaximaAFulles();
...
private:
    ...
    //mètode auxiliar del mètode esArbreSimetric per implementar de forma recursiva
    bool esSimetricAux(BinaryTreeNode<K, V> *node1, BinaryTreeNode<K, V> *node2);

    //mètode auxiliar per fer la suma de tots els camins possibles
    void sumaAux(BinaryTreeNode<K,V> *node, int &max);
};

template<class K, class V>
BinarySearchTree<K, V>::BinarySearchTree() {
    p_root = nullptr;
    cout << "Arbre binari creat\n";
}
...
BinarySearchTree<K, V>::BinarySearchTree(const BinarySearchTree &orig) {
    ...
    cout << "Arbre binari copiat\n";
}

template<class K, class V>
BinarySearchTree<K, V>::~BinarySearchTree() {
    //crida al destructor del BinaryTreeNode
    cout << "\nDestruïnt arbre binari\n";
    delete p_root;
    cout << "...\n";
...
template <class K, class V>
bool BinarySearchTree<K,V>::esArbreSimetric(){
    if(this->p_root == nullptr || (!this->p_root->hasLeft()) && !this->p_root->hasRight()){
        return true;
    } else{
        return esSimetricAux(this->p_root->left(), this->p_root->right());
    }
}

template <class K, class V>
bool BinarySearchTree<K,V>::esSimetricAux(BinaryTreeNode<K,V>* node1, BinaryTreeNode<K,V>* node2){
    if(node1 == nullptr && node2 == nullptr){
        return true;
    } else if(node1 && node2){
        return esSimetricAux(node1->left(), node2->right()) && esSimetricAux(node1->right(), node2->left());
    } else{
        return false;
    }
}
...
template <class K, class V>
int BinarySearchTree<K,V>::sumaMaximaAFulles(){
    if(this->p_root == nullptr){
        return 0;
    } else{
        int max = 0;
        sumaAux(this->p_root, max);
        return max;
    }
}

template <class K, class V>
void BinarySearchTree<K,V>::sumaAux(BinaryTreeNode<K,V>* node, int &max){
    int maxAux = 0;

    if(node->isLeaf()){
        BinaryTreeNode<K,V> *aux = node;
        while(aux != nullptr){
            maxAux += aux->getKey();
            aux = aux->parent();
        }
    ...
    if(node->hasRight()){
        sumaAux(node->right(), max);
    }
    if(node->hasLeft()){
        sumaAux(node->left(), max);
    }
}
