...
public:
    ...
    //realitza la operació mirall sobre l'arbre
    void mirall();
    
    //mostra les claus (keys) dels nodes que estan a nivell k
...
private:
    ...
    //mètode auxiliar del mètode mirall per implementar de forma recursiva
    void mirallAux(BinaryTreeNode<K,V> *node);
    
    //mètode auxiliar del mètode mostraNodesNivell per fer la recursivitat
...
template <class K, class V>
void BinarySearchTree<K,V>::mirallAux(BinaryTreeNode<K,V>* node){
    if(node == nullptr){
        return;
    }
    
    mirallAux(node->left());
    mirallAux(node->right());
...
void BinarySearchTree<K,V>::mostraNodesNivell(int k){
    if(k == height()){
        ...
        throw invalid_argument("BTS is empty!");
    }
    else{
        int L = 0;
        mostraNodesNivell(p_root, k, L);
    }
}
...
void BinarySearchTree<K,V>::mostraNodesNivell(BinaryTreeNode<K,V>* node, int k, int L){
    ...
    if(k == L){
        cout << "nivell " << k << " key " << node->getKey() << endl;
    } else{
        mostraNodesNivell(node->left(), k, L+1);
        mostraNodesNivell(node->right(), k, L+1);
    }
}
