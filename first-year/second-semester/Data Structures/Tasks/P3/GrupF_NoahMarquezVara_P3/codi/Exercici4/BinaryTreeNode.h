...
public:
    ...
    const int &getFactorBalanc() const;
...
template <class K, class V>
void BinaryTreeNode<K, V>::calculateBalanceHeight() {
    altura = 1 + max(this->left()->height(), this->right()->height());
    balanceFactor = (this->right()->height())-(this->left()->height());
}

template <class K, class V>
const int &BinaryTreeNode<K, V>::getFactorBalanc() const {
